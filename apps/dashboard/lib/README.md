...
`middleware.ts` only sees cookies; React needs live token state for API calls. The auth context bridges those two. Feature components under `components/apps/dashboard/` typically consume the context rather than parsing tokens themselves.
