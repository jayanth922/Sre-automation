...
// Types mirror src/backend/schemas.py exactly so the UI renders real data.
...
const INCIDENT_STATUS_BADGE: Record<IncidentStatusT, { label: string; cls: string }> = {
  "open": { label: "Open", cls: "crit" },
  "investigating": { label: "Investigating", cls: "sel" },
  "investigated": { label: "Investigated — no change", cls: "warn" },
  "awaiting_approval": { label: "Awaiting approval", cls: "warn" },
  "remediation_in_progress": { label: "Remediation running", cls: "sel" },
  "remediation_failed": { label: "Remediation failed", cls: "crit" },
  "verification_unknown": { label: "Verification unknown", cls: "warn" },
  "pending_acknowledgment": { label: "Fix verified — awaiting ack", cls: "warn" },
...
export function statusBadge(s: IncidentStatusT, summary?: string | null): { label: string; cls: string } {
  ...
  return INCIDENT_STATUS_BADGE[s]
}
