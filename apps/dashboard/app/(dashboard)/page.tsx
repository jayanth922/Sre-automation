...
export default function HomeGate() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setLoadError("Could not load your clusters. Check the API connection and try again.")
      setClusters([])
    }
  }, [])
    ...
    return (
      <div style={shellStyle}>
        <div style={{ width: "100%", maxWidth: 520 }}>
          <div className="sx-wordmark" style={{ fontSize: 22, marginBottom: 18 }}>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 16 }}>
            <button className="sx-btn primary" onClick={() => void load()}>Retry</button>
            <button className="sx-btn" onClick={logout}>Sign out</button>
          </div>
        </div>
      </div>
    )
