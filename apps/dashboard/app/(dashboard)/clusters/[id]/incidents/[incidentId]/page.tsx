...
export default function IncidentConsolePage() {
  ...
  // The durable incident row remains authoritative when the graph or
  // checkpointer is temporarily unavailable, so the approval cue must not
  // disappear merely because `/status` cannot inspect the interrupt.
  const awaitingApproval = status?.status === "WAITING_APPROVAL" || inc.status === "awaiting_approval"
          ...
          {gates.length > 0 && (
            <div className="sx-remedy">
              ...
              {pendingGates.length > 0 && (
                <div className="sx-dry">
                  {`Approve or deny from the incident's Slack thread ("approve ${pendingGates[0].gate.replace(/_/g, "-")}" / "deny ${pendingGates[0].gate.replace(/_/g, "-")}").`}
                </div>
              )}
            </div>
          )}
