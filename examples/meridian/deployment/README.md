...
# platform. Base installs (`infra/k8s`, Helm `values.yaml`, edge Compose) must
...
3. Sentinel already installed (Helm or `infra/k8s`).

## Helm

```bash
helm upgrade --install sentinel infra/helm/sentinel -n sentinel \
  -f examples/meridian/deployment/helm-values.yaml \
...
```bash
kubectl apply -k examples/meridian/deployment
```
...
In `services/edge_mcp_servers/.env`:
