"""Multi-tenant secure access: Slack OAuth install flow (Phase 4).

An authenticated "start" endpoint mints a short-lived, signed state token
(via ``backend.auth.create_access_token``) that carries the org identity,
and an unauthenticated "callback" endpoint — hit directly by Slack's
redirect, so it cannot carry a bearer token — decodes that state token
instead of relying on a session. This needs no new server-side storage and
works across multiple API workers.
"""
import logging
import uuid
