...
def wrap_tool_with_namespace_scope(tool: Any, context: Any) -> Any:
    """Force authorized cluster namespace onto scoped MCP tool calls."""
    from .namespace_scope import NamespaceScopeError, enforce_tool_arguments

    tool_name = getattr(tool, "name", "unknown")
    original_invoke = getattr(tool, "invoke", None)
    original_ainvoke = getattr(tool, "ainvoke", None)

    def _scoped_args(args: Any) -> dict:
        if args is None:
            payload = {}
            ...
            payload = {"input": args}
        return enforce_tool_arguments(tool_name, payload, context)

    if original_invoke is not None:

        @functools.wraps(original_invoke)
        def scoped_invoke(args=None, **kwargs):
            try:
                return original_invoke(_scoped_args(args), **kwargs)
            except NamespaceScopeError as exc:
                logger.warning("Blocked out-of-scope tool call %s: %s", tool_name, exc)
                raise
    ...
    if original_ainvoke is not None:
        ...
        @functools.wraps(original_ainvoke)
        async def scoped_ainvoke(args=None, **kwargs):
            try:
                return await original_ainvoke(_scoped_args(args), **kwargs)
            except NamespaceScopeError as exc:
                logger.warning("Blocked out-of-scope tool call %s: %s", tool_name, exc)
                raise
...
def wrap_all_tools_with_retry(
    ...
    *,
    ...
    3. Audit Logic (Outer)
    4. Namespace scope (optional, when execution_context is provided)

    Args:
        tools: List of LangChain BaseTool instances
        max_attempts: Maximum retry attempts per tool call
        execution_context: Optional tenant ExecutionContext for namespace enforcement
    ...
    for tool in tools:
        ...
        if execution_context is not None:
            audit_tool = wrap_tool_with_namespace_scope(audit_tool, execution_context)
        wrapped_tools.append(audit_tool)

    suffix = " + NamespaceScope" if execution_context is not None else ""
    logger.info(
        "Wrapped %s tools with Retry + CircuitBreaker + Audit%s",
        len(wrapped_tools),
        suffix,
    )
