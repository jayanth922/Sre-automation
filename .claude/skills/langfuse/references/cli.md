...
Documentation: https://langfuse.com/docs/api-and-data-infra/local/features/cli
...
- Always query via `observations`, not `traces`, even when the user phrases the request in terms of traces — the `traces` endpoints are outdated (add `--trace-id` to scope to a known trace). See the [Observations API docs](https://langfuse.com/docs/api-and-data-infra/local/features/observations-api) for the v1 → v2 mapping.
