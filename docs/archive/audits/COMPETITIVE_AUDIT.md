...
> **Point-in-time audit.** Product comparisons and repository findings may no
> longer be current. This file is retained for historical context only.
...
- So: keep the dashboard (incident timeline, audit trail, and the **plan
  ...
  **FastAPI core API-first** so apps/dashboard/CLI/chat are all just clients. For a
...
| **Durability / resume** | **Temporal**, **LangGraph checkpointer** (what we use), Restate | **Comparable design** — using the standard LangGraph checkpointer for OODA-loop resume is the idiomatic choice. Temporal itself is also wired, scoped narrower: `src/sre_agent/temporal_client.py` orchestrates the code-fix verification sandbox (`sandbox_workflow.CodeFixVerificationWorkflow`), with either a lightweight local dev-server (`temporalio/temporal:latest server start-dev`, opt-in via `COMPOSE_PROFILES=local-temporal`) or Temporal Cloud (`TEMPORAL_HOST`/`TEMPORAL_API_KEY`) — not a general task-queue replacement for the checkpointer. |
| **Agent observability** | **Langfuse** (most-adopted OSS), **Arize Phoenix**, **LangSmith**, **AgentOps** | **Comparable design** — real Langfuse `CallbackHandler` (`src/sre_agent/tracing.py`), attached to every graph invocation via `checkpointer.py::thread_config()`, so every LLM/tool/chain span gets tokens, cost, latency, and trajectory automatically; Langfuse Cloud only (free tier) — no self-hosted option, so no clickhouse/minio/web/worker containers to run. No Phoenix/AgentOps alternative backend, and OpenTelemetry export isn't wired, but this is the real most-adopted-OSS tool itself, not a homegrown recorder. |
...
| **Secure multi-tenant access** | HolmesGPT/Robusta relay; egress-only agents (Plural, Atlan) | **Comparable design** — built (PR #49, 2026-08-30): `src/sre_agent/multitenant/` mints per-tenant GitHub App installation tokens and Slack OAuth tokens instead of one shared static token, relayed edge-side via `X-Sentinel-Relay-*` headers (`relay_auth.py`/`services/edge_mcp_servers/relay_credentials.py`) with fallback to the old single-tenant env vars. Simpler than a full egress-only network boundary (Plural/Atlan), but no longer just specified. |
...
1. **ITBench adapter** (`evals/benchmarks/itbench_adapter.py`) — maps our output to the
   IBM ITBench SRE diagnosis shape for real, comparable numbers.
2. **Langfuse tracing** (`src/sre_agent/tracing.py`) — Langfuse v3 CallbackHandler
   wired into the investigation config; enable with `LANGFUSE_PUBLIC_KEY`.
3. **LiteLLM router backend** (`src/sre_agent/litellm_backend.py`) — `route_llm`
   ...
   top.
4. **Toolset breadth** (`src/sre_agent/toolsets.py`) — explicit registry of the 7
...
(`src/sre_agent/code_sandbox.py`), was built then removed (2026-09-04): it had zero
production callers — the live code-fix path is `sandbox_workflow.py`'s
Temporal-orchestrated K8s Job (`services/edge_mcp_servers/mcp_servers/sandbox_real/`),
