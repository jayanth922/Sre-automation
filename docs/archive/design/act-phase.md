...
> **Historical document.** The implementation has evolved beyond this design.
> Use the runtime, tests, and `docs/ai/PROJECT_STATE.md` for current behavior.
...
## 2. Severity model (`src/sre_agent/severity_engine.py`)
...
## 3. Autonomy gate (`src/sre_agent/policy_gate.py`)
...
## 4. Executor (`src/sre_agent/executor.py`)
...
- **Live (`aexecute`, async):** calls the **Executor MCP server**
  (`services/edge_mcp_servers/mcp_servers/executor_real`, port 4005) through an injected
