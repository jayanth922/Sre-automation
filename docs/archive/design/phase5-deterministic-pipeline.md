...
> **Historical document.** This plan was implemented and superseded. Its draft
> status below is preserved as a point-in-time record, not current project state.
...
| Piece | File | Verdict |
|---|---|---|
| Sandbox fix verification (baseline vs. patched log diff) | `src/sre_agent/sandbox_workflow.py::CodeFixVerificationWorkflow` | **Reuse as-is.** Already exactly the "apply in Temporal, check again, RESOLVED/REGRESSED/INCONCLUSIVE" oracle requirement 1 asks for. Currently triggered fire-and-forget *after* live execution (`graph_builder.py:287-352`) as a confirmation, not *before* it as a gate — needs to move earlier in the flow. |
| Dashboard real-time transport | `apps/dashboard/lib/useLiveStream.ts` | **Reuse as-is.** Exponential-backoff reconnect, fresh ticket per attempt, cursor-based replay on reconnect, client dedup by event id. Already satisfies requirement 3's dashboard half. |
| Slack transport | `src/sre_agent/integrations/slack_bot.py` (Socket Mode, `AsyncSocketModeHandler`) | **Reuse, audit only.** Library-level reconnect exists; not yet stress-tested against a 30+ minute approval wait (current known blocker: approval requests expire ~30 min — see `PROJECT_STATE.md`). |
| Per-issue chat isolation | `src/sre_agent/mission_control.py`, `war_room.py` | **Reuse.** Incident-scoped threading already exists; keying everything (thread, workflow, PR) off one `workflow_id` per bundle gets requirement 4 for free. |
| Exact-title incident dedup | `src/sre_agent/api/v1/alerts.py:241`, `crud.find_duplicate_incident` | Exists but is **not** correlation — exact title match only, same cluster. No root-cause grouping across different alert titles. |
| GitHub PR creation | `services/edge_mcp_servers/mcp_servers/github_exec/server.py::create_revert_pr` | **Partial.** A PR-opening tool exists (`gh pr revert`, dry-run-by-default, typed CREATED/MANUAL_REQUIRED/ERROR statuses, guardrailed) but only for reverting an existing merged commit/PR — there is no tool to open a PR carrying an arbitrary AI-generated patch/diff. Phase C is "add a `create_fix_pr` tool next to `create_revert_pr`," not a from-scratch build. |
| Concurrent-issue correlation/bundling | `src/sre_agent/incident_correlation.py` | **Done (Phase A, shadow mode) — see below.** |
...
- **Phase A — Correlation engine. DONE (2026-09-02), shadow mode.**
  `src/sre_agent/incident_correlation.py::correlate` — pure, deterministic,
  ...
  `src/sre_agent/api/v1/alerts.py`'s webhook handler as `_record_correlation_shadow`,
...
- **Phase B — `IncidentRemediationWorkflow` skeleton + both approval
  signals. DONE (2026-09-02).** `src/sre_agent/incident_remediation_workflow.py`
  ...
  *between* the two gates (Phase 5A's reordering: sandbox-verify before any
  ...
  (`src/sre_agent/api/v1/remediation_gates.py`, admin-gated) list and CAS-decide
...
- **Phase C — GitHub PR creation. DONE (2026-09-02), folded into Phase B.**
  `services/edge_mcp_servers/mcp_servers/github_exec/server.py::create_fix_pr` —
...
- **Phase D — Wire Slack + dashboard to the two new gates. DONE (2026-09-02).**
  Dashboard: `apps/dashboard/.../incidents/[incidentId]/page.tsx` gained a
...
- **Phase F — AI patch generation (actor-runtime). Implemented, committed
  ...
  the params directly. Separately, `src/sre_agent/actor_runtime.py`'s
  ...
  invoked by `evals/benchmarks/terminal_bench_adapter.py`) — confirmed dead from
