...
| **Durability / resume** | **Temporal**, **LangGraph checkpointer** (what we use), Restate | **Comparable design** — using the standard LangGraph checkpointer for OODA-loop resume is the idiomatic choice. Temporal itself is also wired, scoped narrower: `sre_agent/temporal_client.py` orchestrates the code-fix verification sandbox (`sandbox_workflow.CodeFixVerificationWorkflow`), with either a lightweight local dev-server (`temporalio/temporal:latest server start-dev`, opt-in via `COMPOSE_PROFILES=local-temporal`) or Temporal Cloud (`TEMPORAL_HOST`/`TEMPORAL_API_KEY`) — not a general task-queue replacement for the checkpointer. |
| **Agent observability** | **Langfuse** (most-adopted OSS), **Arize Phoenix**, **LangSmith**, **AgentOps** | **Comparable design** — real Langfuse `CallbackHandler` (`sre_agent/tracing.py`), attached to every graph invocation via `checkpointer.py::thread_config()`, so every LLM/tool/chain span gets tokens, cost, latency, and trajectory automatically; Langfuse Cloud only (free tier) — no self-hosted option, so no clickhouse/minio/web/worker containers to run. No Phoenix/AgentOps alternative backend, and OpenTelemetry export isn't wired, but this is the real most-adopted-OSS tool itself, not a homegrown recorder. |
