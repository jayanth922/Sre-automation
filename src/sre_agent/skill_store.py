...
actor backend (see src/sre_agent/actor_runtime.py).
...
# enabling this costs no extra infra (see infra/local/docker-compose.yaml).
...
# production embedding model (evals/benchmarks/calibrate_semantic_floor.py), the two
...
class SemanticSkillStore(JsonSkillStore):
    ...
    to make the platform default (mirrors src/sre_agent/litellm_backend.py's
