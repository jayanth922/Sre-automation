...
Pipeline:
    0. (Phase F, only when the planner didn't supply a patch) generate_patch_activity
       — the pluggable actor runtime (src/sre_agent/actor_runtime.py) proposes a
...
def _repo_clone_url(repo: str, token: str = "") -> str:
    ...
    — see services/edge_mcp_servers/mcp_servers/github_real/README.md). Pure so the
...
async def generate_patch_activity(params: IncidentRemediationInput) -> PatchGenerationResult:
    ...
    (src/sre_agent/actor_runtime.py) against a throwaway clone of the incident's
...
async def raise_pr_activity(params: IncidentRemediationInput) -> PrResult:
    ...
    (services/edge_mcp_servers/mcp_servers/github_exec/server.py) but for an arbitrary
...
async def open_gate_activity(
    ...
    apps/dashboard/API has nothing to CAS-decide against.
    """
