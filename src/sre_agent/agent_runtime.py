...
# Must precede the `backend.*` imports below: src/backend/auth.py reads SECRET_KEY
...
async def agent_metrics():
    ...
    # `evals/benchmarks/retrieval_eval.py` carries the labeled half. What this does
...
# WebSocket ticket minting (see src/sre_agent/ws_auth.py)
...
async def _run_graph_impl(
    ...
    try:
        ...
        # resolved the incident (see src/sre_agent/incident_status.py).
        act_report = (current_execution_state.get("metadata") or {}).get("act_report")
        verification_outcome = (act_report or {}).get("verification")
