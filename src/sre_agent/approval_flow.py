...
# running IncidentRemediationWorkflow (src/sre_agent/incident_remediation_workflow.py).
...
async def create_or_reuse_pending_gate_approval(
    *,
    ...
    """Persist one gate's PENDING state as a durable row so the apps/dashboard/API
...
async def decide_and_signal_gate(
    *,
    ...
    src/sre_agent/api/v1/remediation_gates.py, Slack in war_room.py) so they
...
async def acknowledge_incident_resolution(
    *,
    ...
    "acknowledge" command (src/sre_agent/war_room.py) so neither surface can
