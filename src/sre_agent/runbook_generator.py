...
see `src/sre_agent/notion_runbooks.py::upsert_notion_runbook`). That closes a
learning loop: the Planner's runbook RAG (`search_runbooks`, served by
`services/edge_mcp_servers/mcp_servers/runbooks_notion`) then finds the agent's own
