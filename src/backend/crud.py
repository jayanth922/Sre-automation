...
async def get_clusters_for_org(db: AsyncSession, org_id: uuid.UUID):
    result = await db.execute(
        select(models.Cluster).filter(models.Cluster.org_id == org_id)
    )
    return result.scalars().all()


async def create_cluster(
    db: AsyncSession, cluster: schemas.ClusterCreate, org_id: uuid.UUID
):
...
async def update_cluster(
    ...
    if "metrics_config" in data:
        cluster.metrics_config = (
            _json.dumps(data["metrics_config"]) if data["metrics_config"] else None
        )
    await db.commit()
...
async def get_cluster_by_id(db: AsyncSession, cluster_id: uuid.UUID):
    result = await db.execute(
        select(models.Cluster).filter(models.Cluster.id == cluster_id)
    )
    return result.scalars().first()


async def update_cluster_heartbeat(
    db: AsyncSession,
    cluster_id: uuid.UUID,
    *,
    source: str,
    ...
    """Record an observed connectivity signal and derive status from it."""
    from sre_agent.cluster_heartbeat import evaluate_heartbeat

    now = datetime.now(timezone.utc)
    evaluation = evaluate_heartbeat(now, source=source, now=now)
    stmt = (
        models.Cluster.__table__.update()
...
async def reconcile_cluster_heartbeats(db: AsyncSession) -> int:
    ...
    now = datetime.now(timezone.utc)
    for cluster in clusters:
        if cluster.status == models.ClusterStatus.MAINTENANCE:
            continue
        evaluation = evaluate_heartbeat(
            cluster.last_heartbeat,
            source=cluster.heartbeat_source,
            now=now,
        )
        if (
            cluster.status == evaluation.status
            and (cluster.heartbeat_reason or None) == evaluation.reason
        ):
            continue
        cluster.status = evaluation.status
        cluster.heartbeat_reason = evaluation.reason
        updated += 1
    if updated:
        await db.commit()
    return updated
...
async def list_active_incidents_for_cluster(
    ...
    `src/sre_agent/incident_correlation.py::correlate` when a new incident fires.
    """
    filters = [
        models.Incident.cluster_id == cluster_id,
        models.Incident.status.in_(_ACTIVE_INCIDENT_STATUSES),
...
async def get_incident_timeline_events(
    ...
    result = await db.execute(
        ...
        .order_by(
            models.IncidentTimelineEvent.sequence.asc(),
            models.IncidentTimelineEvent.created_at.asc(),
        )
    )
    return result.scalars().all()
...
async def get_incidents_for_cluster(db: AsyncSession, cluster_id: uuid.UUID):
    result = await db.execute(
        select(models.Incident)
        .filter(models.Incident.cluster_id == cluster_id)
        .order_by(models.Incident.created_at.desc())
    )
    return result.scalars().all()
...
async def delete_cluster(
    ...
    result = await db.execute(
        select(models.Cluster).filter(
            models.Cluster.id == cluster_id, models.Cluster.org_id == org_id
        )
    )
...
async def create_slo(
    db: AsyncSession, cluster_id: uuid.UUID, slo: schemas.SLOCreate
) -> models.SLO:
    db_slo = models.SLO(
        cluster_id=cluster_id,
        name=slo.name,
...
async def get_slos_for_cluster(
    db: AsyncSession, cluster_id: uuid.UUID
) -> List[models.SLO]:
    result = await db.execute(
...
async def revoke_refresh_session(
    db: AsyncSession, session: "models.RefreshSession"
) -> None:
