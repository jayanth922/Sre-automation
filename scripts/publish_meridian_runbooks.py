...
The four pages in `runbooks/meridian/` score 22/22 on
`scripts/audit_runbook_coverage.py` against the v2 scenarios, where the live
...
After a real publish, verify what Notion actually serves:
    ...
    python scripts/dump_notion_runbook_corpus.py --cluster-id <uuid> --out /tmp/after.json
    python scripts/audit_runbook_coverage.py --corpus /tmp/after.json

That, not the local 22/22, is the number that says Phase B is live.

Usage:
    python scripts/publish_meridian_runbooks.py --cluster-id <uuid>
    python scripts/publish_meridian_runbooks.py --cluster-id <uuid> --confirm
"""
...
DEFAULT_DRAFTS = REPO / "runbooks" / "meridian"
...
async def run(args: argparse.Namespace) -> int:
    ...
    if not args.confirm:
        print(
            ...
            "  python scripts/dump_notion_runbook_corpus.py --out /tmp/after.json ...\n"
            "  python scripts/audit_runbook_coverage.py --corpus /tmp/after.json"
        )
        return 0
