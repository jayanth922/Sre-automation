...
check_no_meridian "$root/infra/k8s/config.yaml"
check_no_meridian "$root/infra/k8s/rbac.yaml"
check_no_meridian "$root/infra/helm/sentinel/values.yaml"

# Compose may mention Meridian only as documentation pointing at the example.
if grep -n 'EXECUTOR_ALLOWED_NAMESPACES=meridian' "$root/services/edge_mcp_servers/docker-compose.yaml" >/dev/null 2>&1; then
...
if ! grep -ri 'meridian' "$root/examples/meridian/deployment" >/dev/null; then
  echo "examples/meridian/deployment overlay missing Meridian contract content" >&2
  fail=1
fi
...
if command -v kubectl >/dev/null 2>&1; then
  stub=0
  if [[ ! -f "$root/infra/k8s/secret.yaml" ]]; then
    stub=1
    cat >"$root/infra/k8s/secret.yaml" <<'EOF'
apiVersion: v1
kind: Secret
...
EOF
  ...
  render="$(mktemp)"
  kubectl kustomize "$root/infra/k8s" >"$render"
  ...
  rm -f "$render"
  if [[ "$stub" -eq 1 ]]; then
    rm -f "$root/infra/k8s/secret.yaml"
  fi
fi
