...
`src/sre_agent/ablation.py` defines four measurement configurations, selected with
...
`evals/benchmarks/` is not in the agent image, so copy it in for the run and take it
...
docker exec sre-agent-api /app/.venv/bin/python \
  /app/evals/benchmarks/ablation_coverage.py \
...
```bash
uv run python evals/benchmarks/statistical_smoke_dataset.py \
...
```bash
uv run python evals/benchmarks/ablation_eval.py reports/ablation-trials.jsonl \
