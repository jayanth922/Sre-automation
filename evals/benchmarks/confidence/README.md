...
```bash
python evals/benchmarks/confidence_eval.py reports/sre-bench-confidence.jsonl \
  --task remediation \
...
`synthetic`. `evals/benchmarks/sre_bench.py` is the only sanctioned producer of
