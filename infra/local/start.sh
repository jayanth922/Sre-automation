...
# without it (see src/sre_agent/provider_config.py) and it's set per-cluster
...
# The edge MCP stack is a second, independently deployable Docker Compose
# project with its own .env. scripts/dev/start.sh brings it up directly, so
# bootstrap it here too, and sync MCP_SERVICE_TOKEN so the two stacks trust
# each other without a manual copy-paste.
EDGE_DIR="$repo_root/services/edge_mcp_servers"
if [ ! -f "$EDGE_DIR/.env" ] && [ -f "$EDGE_DIR/.env.example" ]; then
    cp "$EDGE_DIR/.env.example" "$EDGE_DIR/.env"
    echo -e "${GREEN}✅ services/edge_mcp_servers/.env created.${NC}"
    echo -e "${YELLOW}   Set a real GITHUB_TOKEN (and GITHUB_REPO) in services/edge_mcp_servers/.env before continuing.${NC}"
fi
...
if [ -f "$EDGE_DIR/.env" ] && grep -qE '^MCP_SERVICE_TOKEN=$' "$EDGE_DIR/.env"; then
    ...
    if [ -n "$MCP_TOKEN_VALUE" ]; then
        ...
        echo -e "${GREEN}✅ services/edge_mcp_servers/.env MCP_SERVICE_TOKEN synced from platform .env.${NC}"
    fi
fi
...
# regardless of whether it is invoked from the repo root or infra/local/.
set -a
...
if ! (
    cd "$repo_root"
    PYTHONPATH="$repo_root/src" python3 -m sre_agent.provider_config
); then
    echo -e "${RED}❌ Startup configuration is invalid. Fix .env (see messages above) and retry.${NC}"
    echo -e "${YELLOW}   Supported LLM_PROVIDER: anthropic${NC}"
    exit 1
fi
...
echo -e ""
echo -e "   👉 To connect a cluster's tools (Prometheus/Loki/GitHub/runbooks/exec): see services/edge_mcp_servers/"
echo -e "   👉 To stop: ./stop.sh"
echo -e "   👉 Logs: docker compose --env-file .env -f infra/local/docker-compose.yaml logs -f sre-agent-api"
echo -e ""
