...
A **Helm release wrapper** for the in-repo chart at `infra/helm/sentinel`.
...
```bash
# 1) Ensure images exist on the cluster (see infra/helm/sentinel/README.md)
...
`scripts/ci/check_terraform.sh` runs `fmt`, `validate`, and a representative
