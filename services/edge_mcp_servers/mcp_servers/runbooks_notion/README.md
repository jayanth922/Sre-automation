...
- Per-cluster: `notion_api_key` / `notion_database_id`, relayed per request from the control plane (`X-Sentinel-Relay-Notion-Key` / `X-Sentinel-Relay-Notion-Database` — see `src/sre_agent/multitenant/relay_auth.py`).
